...
async function enviarChat() {
    ...
    try {
        const res = await fetch(`${API_URL}/api/briefing`, { // <-- CORREGIDO
            method: 'POST',
            headers: {
...
sellarJuicioBtn.addEventListener('click', async () => {
    ...
    try {
        await fetch(`${API_URL}/api/gandalf/judge`, {
            method: 'POST',
...
window.juicioGandalf = async (id, veredicto) => {
    ...
    try {
        ...
        const res = await fetch(`${API_URL}/api/gandalf/judge`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
function cargarLogrosUsuario(unlockedIds) {
    ...
    obtenerToken().then(token => {
        fetch(`${API_URL}/api/achievements/progress`, {
            headers: { 'Authorization': `Bearer ${token}` }
        })
            .then(res => res.json())
            .then(data => {
...
async function seleccionarRaza(raza) {
    ...
    try {
        const endpoint = raceSelectionMode === 'change'
            ? `${API_URL}/api/profile/change-race`
            : `${API_URL}/api/profile/select-race`;
...
if (changeRaceBtn) {
    changeRaceBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!profile?.race) {
                showToast('Debes elegir una raza inicial antes de continuar.', 'error');
                openRaceSelectionModal('onboarding');
            } else {
                // Si ya tiene raza, forzar el cierre del modal si está abierto
                raceSelectionModal.classList.add('hidden');
                raceModalShownOnce = true;  // Asegurarse de que no vuelva a abrirse
            }
        } catch (err) {
            console.error("Error al verificar raza del usuario:", err);
        }
    });
}

// ===================================================================================
// LÓGICA DEL JUEGO Y MUNDO
// ===================================================================================
async function cargarMisiones(fecha) {
    const token = await obtenerToken();
    if (!token) return;

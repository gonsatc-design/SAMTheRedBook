...
// --- Datos de Prueba ---
// Simulamos una respuesta de la DB con datos para 2 de los últimos 7 días
...
describe('Analytics - Agregador de Snapshot Histórico', () => {
    ...
    test('Debe devolver siempre un array con 7 registros (uno por cada día)', async () => {
        const snapshot = await getHistoricalSnapshot('test-user');
        expect(snapshot).toHaveLength(7);
    });

    test('Debe devolver valores neutrales (0) para un día sin actividad', async () => {
        const snapshot = await getHistoricalSnapshot('test-user');
        // El día de anteayer no está en nuestro mock, así que debería ser neutral
    ...
    test('Debe calcular correctamente los porcentajes y estructurar los datos para un día con actividad', async () => {
        const snapshot = await getHistoricalSnapshot('test-user');
    ...
    test('Debe manejar correctamente un día con 0% de éxito', async () => {
        const snapshot = await getHistoricalSnapshot('test-user');
